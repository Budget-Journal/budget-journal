...
router.put("/", rejectUnauthenticated, (req, res) => {
  ...
  const queryText = `
        ...
        WHERE "user".id=$2
    `;
  let queryParams = [req.body.total_budget, req.user.id];
  pool
...
router.put("/add_to_budget", rejectUnauthenticated, (req, res) => {
  ...
  const queryText = `
        ...
        WHERE "user".id=$2
    `;
  let queryParams = [req.body.sum, req.user.id];
  pool
    .query(queryText, queryParams)
    .then((result) => {
      res.sendStatus(201);
...
router.put("/subtract_from_budget", rejectUnauthenticated, (req, res) => {
  ...
  const queryText = `
        ...
        WHERE "user".id=$2
    `;
  let queryParams = [req.body.difference, req.user.id];
  pool
    .query(queryText, queryParams)
    .then((result) => {
      res.sendStatus(201);
...
router.put("/on_completed_goal", rejectUnauthenticated, (req, res) => {
  ...
  const queryText = `
        ...
        WHERE "user".id=$2
    `;
  let queryParams = [req.body.updateBudget, req.user.id];
  pool
    .query(queryText, queryParams)
    .then((result) => {
      res.sendStatus(201);
